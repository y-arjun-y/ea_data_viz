...
countries.loc[
    countries["Country"] == "United States of America", "Country"
] = "United States"
...
countries["Density (per million)"] = (
    countries["Responses"] / countries["population"] * 1e6
)
countries["Density (per million)"] = countries["Density (per million)"].apply(
    lambda x: round(x, 2)
)
countries["log density"] = countries["Density (per million)"].apply(
    lambda x: 1 + log(x + 1)
)
...
countries_for_map.loc[
    len(countries_for_map),
    ["Country", "Responses", "Density (per million)", "log density"],
] = ("Antarctica", 0, 0, 0)
...
countries_capita_sort["text"] = countries_capita_sort["Density (per million)"].apply(
    lambda x: f"{x:.1f}"
)
...
def country_total_section():
    return html.Div(
        [
            ...
            get_instructions(
                hover="countries or bars", extra_text="Scroll to zoom on map."
            ),
            html.Div(
                html.Div(
                    [
            ...
            html.Div(
                html.Div(
                    [
                        ...
                        html.Div(per_capita_bar, className="plot-container"),
                    ],
                    className="grid desk-cols-2-1",
                ),
                className="section-body",
