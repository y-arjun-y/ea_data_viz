...
def get_forum_data():
    ...
    with open("./assets/data/general_ea/ea_forum.json", "r") as forum_file:
...
def post_counts(forum_df):
    ...
    forum_by_day_df["new_author_count"] = (
        posted_at_groups["new_author_count"].sum().tolist()
    )
    ...
    def new_authors_hover(row):
        ...
        result += f"<b>{posted_at}</b>"
        result += (
            f"<br><b>{new_authors_count} new authors ({total_authors:,} total):</b>"
        )
    ...
    forum_by_day_df["new_authors_hover"] = forum_by_day_df.apply(
        new_authors_hover, axis=1
    )

    # word count

    forum_by_day_df["new_words"] = posted_at_groups["wordcount"].sum().tolist()
    forum_by_day_df["total_words"] = (
        posted_at_groups["cumulative_word_count"].last().tolist()
    )
    ...
    forum_by_day_df["word_count_hover"] = forum_by_day_df.apply(
        word_count_hover, axis=1
    )
...
def forum_user_wilkinson_section():
    ...
    forum_df["first_author"] = forum_df["authors"].apply(
    ...
    author_df["posted_at_readable"] = (
        author_groups["posted_at_readable"].first().tolist()
    )
    ...
    return html.Div(
        [
            html.Div(
                html.H2(
                    "EA Forum Authors: Total Karma, Total Wordcount, and Date of First Post"
                ),
                className="section-title",
            ),
            get_instructions(hover="points", zoom=True),
