...
doing = doing.append(
    commiting.loc[commiting["Type of data"] == "Founder’s Pledge pledges"],
    ignore_index=True,
)
...
for df in growing_dfs:
    ...
    df.columns = [col.replace("Jan-Dec ", "") for col in df.columns]
    ...
    def field_to_numeric(field):
        ...
        field = "".join([char for char in field if char in valid_chars])
        return eval(field)
        return field
