...
def get_op_grants():
    ...
    op_grants["Focus Area"] = op_grants["Focus Area"].apply(
        lambda x: x.replace("Artificial Intelligence", "AI")
    )
    ...
    op_grants["Organization Name"] = op_grants["Organization Name"].apply(
        normalize_orgname
    )
...
def openphil_grants_categories_section():
    ...
    return html.Div(
        [
            html.Div(
                html.H2(
                    "Open Philanthropy Grants by Focus Area and Donee Organization"
                ),
                className="section-title",
            ),
            get_instructions(),
...
def group_by_month(op_grants):
    ...
    for i, date in enumerate(dates):
        ...
        grants_by_month.loc[i, "organizations"] = grants_by_month_i[
            "Focus Area"
        ].tolist()
